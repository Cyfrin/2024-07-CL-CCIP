...
contract EVM2EVMOnRamp is IEVM2AnyOnRamp, ILinkAvailable, AggregateRateLimiter, ITypeAndVersion {
  ...
  /// @dev Same as TokenTransferFeeConfig but with the token included so that an array of these can be passed in
  /// to setTokenTransferFeeConfig
  ...
  // STATE
  /// @dev The current nonce per sender. The offramp has a corresponding s_senderNonce mapping to ensure messages
  /// are executed in the same order they are sent.
  mapping(address sender => uint64 nonce) internal s_senderNonce;
  /// @dev The amount of LINK to pay to NOPS
  uint96 internal s_nopFeesJuels;
  /// @dev The combined weight of all NOPs weights
  uint32 internal s_nopWeightsTotal;
  /// @dev The last used sequence number. This is zero in the case where no messages has been sent yet. 0 is
  /// not a valid sequence number for any real transaction.
  ...
  function forwardFromRouter(
    ...
    // Router address may be set to zero intentionally to pause.
    ...
    if (message.feeToken == i_linkToken) {
      ...
      // the cast from uint256 to uint96 is considered safe, uint96 can store more than max supply of the link token
    ...
    Internal.EVM2EVMMessage memory newMessage = Internal.EVM2EVMMessage({
      ...
      // Not validated in `getFee`. Invalid address is uncommon, gas cost outweighs UX gain.
      receiver: Internal._validateEVMAddress(message.receiver),
      sequenceNumber: ++s_sequenceNumber,
      gasLimit: extraArgs.gasLimit,
      // Always false since this functionality has been removed. Still in the event to maintain backwards compatibility.
      strict: false,
      ...
      messageId: "" // will be populated below
    });
  ...
  /// @dev This function can be called multiple times during a CCIPSend, only common user-driven mistakes
  /// are validated here to minimize duplicate validation cost.
  ...
  /// @inheritdoc IEVM2AnyOnRampClient
  /// @dev This function has been removed, but needs to remain available for backwards compatibility reasons.
  ...
  /// @return totalNopFees
  function getNopFeesJuels() external view returns (uint96 totalNopFees) {
